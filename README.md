...
candidate floor 4.0 / net-σ floor 4.0 / per-arm CNN ranking / blind floor / coherence ceiling 0.999).
...
madgrav/
  ...
  launchers/            # run_search.sh, slurm_search.sbatch, run_merge.sh (56-seg quick run)
  launchers/run_chain.sh + templates/  # generic config-driven full-run launcher (any observing run)
  launchers/chain_*.py   # run-agnostic progress/finalize/notify helpers
  site.conf.example      # per-site config (paths, SLURM, optional email) -> copy to site.conf
```
...
## Full blind-search pipeline (`run_chain.sh`)

For a complete observing-run analysis (O3a / O3b / O4a / O4b) the config-driven launcher runs the whole
chain — `prep → (streams ‖ cache ‖ inject) → background prune → CNN → FAR merge`, plus a parallel
foreground preview — from a **single command**, deriving every per-run path from the run label and the
segment count. All machine-specific values live in **one git-ignored file**:

```bash
cp site.conf.example site.conf          # then edit: MADGRAV_ROOT, SCRATCH, PYTHON,
                                        #   SLURM account/partitions, (optional) NOTIFY_EMAIL + SMTP
python search_mode/build_o4a_coincident.py   # build the coincident manifest -> $SCRATCH/o4a_full_coincident.json
                                             #   (build_o3b_/build_o4b_ for other runs; OUT overridable via SM_BGJSON)
bash launchers/run_chain.sh o4a --dry-run    # print the full plan (paths, shard counts, sbatch cmds); submit nothing
bash launchers/run_chain.sh o4a              # launch (guards on strain completeness)
```

Shard geometry (streams / cache / FAR) is derived from the manifest's segment count; frozen science
config (floor 4.0, coherence ceiling 0.999, per-arm CNN ranking, held-out `data/o3a_frozen_lr_off200.npz`
statistic) is identical across runs. **Results are always written to `<out>/SUMMARY.txt`** (and the job
log); email is sent only if `NOTIFY_EMAIL` is set in `site.conf` — so anyone gets the results with zero
notification setup.

The optional injection / `p_astro` stage needs two **projected-waveform template banks** under `data/`
— `o1_o3_signal_bank_projected_2s_x10/` (stellar-mass) and `ultramassive_bank/` (high-mass). Each is a
directory of `signals_*.npz` files holding float32 `H1` / `L1` arrays (2 s waveforms projected onto each
detector at 4096 Hz), with an optional per-file `.csv` sidecar (`source_id`, `total_mass`,
`distance_mpc`). They are **not shipped** (multi-GB). Either point `SM_BANK_SIG` / `SM_BANK_UM` at an
existing bank, or run **`run_chain.sh <run> --far-only`** to skip `p_astro` entirely (the FAR search is
fully reproducible without them). See `docs/signal_bank.md` for the regeneration recipe.
...
paths off `MADGRAV_ROOT`, so `export MADGRAV_ROOT=/path/to/your-analysis-tree` reruns them against an
