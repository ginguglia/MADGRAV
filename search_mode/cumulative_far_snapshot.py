"""CUMULATIVE-FAR SNAPSHOT — NON-QUOTABLE diagnostic copy of driver_blindscan_merge.py (2026-07-03).

Provisional FAR BOUNDS from a PARTIAL CNN shard cache while the campaign runs:
  SM_PARTIAL_MODE=lo : unscored bg pairs NEVER count as louder  -> every FAR is a LOWER bound
  SM_PARTIAL_MODE=hi : unscored bg pairs ALWAYS count as louder -> every FAR is an UPPER bound
Reads cnn_shard_*.npz from SM_SHARD_SRC (the live campaign dir), writes ONLY to SM_OUT (a separate
_partial dir). Never scores inline (safe on CPU/login node). The real merge driver is untouched.
...
def main():
    ...
    if _cshard:                                       # WORKER: score my strided slice, save shard, run NOTHING downstream
        ...
        cache=PB.precompute_cnn(mine,segs,B.cpipe,B.carm,B.cnet,B.lmnet,B._win)
        ...
        raise SystemExit(0)
    # ---- PARTIAL-CACHE SNAPSHOT MODE (this copy ONLY): tolerate missing pairs via a bounds sentinel ----
    _PMODE=os.environ.get("SM_PARTIAL_MODE")
    if _PMODE not in ("lo","hi"):
        raise SystemExit("[snapshot] set SM_PARTIAL_MODE=lo (FAR lower bound) or hi (FAR upper bound)")
    # lo: unknown pair can never count as louder / never passes the glitch gate  -> FAR lower bound
    # hi: unknown pair always counts as louder AND always passes the glitch gate -> FAR upper bound
    _SENT=(-1e9,-1e9) if _PMODE=="lo" else (1e9,1e9)
    _SRC=os.environ.get("SM_SHARD_SRC") or OUT
    cnn_cache={}
    for _f in sorted(glob.glob(f"{_SRC}/cnn_shard_*.npz")):
    ...
    _n_scored=len(need_pairs)-len(_miss)
    print(f"[snapshot] PARTIAL CNN cache: {_n_scored}/{len(need_pairs)} need_pairs scored "
          f"({100.0*_n_scored/max(1,len(need_pairs)):.1f}%), mode={_PMODE}",flush=True)
    _unknown_hits=[0]
    def surv_cnn(ai,iH,bi,iL):
        key=(int(ai),int(iH),int(bi),int(iL))
        if key not in cnn_cache: _unknown_hits[0]+=1; return max(_SENT)
        hm,lm=cnn_cache[key]; return max(hm,lm)
    _fallback_hits=[0]
    def surv_cnn_pair(ai,iH,bi,iL):                         # per-arm (hm,lm) lookup; sentinel for unscored pairs
        key=(int(ai),int(iH),int(bi),int(iL))
        if key not in cnn_cache: _unknown_hits[0]+=1; return _SENT
    ...
    print(f"\n=== PARTIAL-{_PMODE.upper()} DETECTIONS (NON-QUOTABLE; point FAR<{DET_FAR}/yr AND UL90<{DET_FAR}/yr; "
          f"stat={'per-arm' if PERARM else 'OR-veto'}): {len(dets)} ===",flush=True)
    for d in dets:
    ...
    json.dump(dict(partial_mode=_PMODE,n_scored=_n_scored,n_needed=len(need_pairs),
                   frac_scored=_n_scored/max(1,len(need_pairs)),unknown_lookups=_unknown_hits[0],
                   note="NON-QUOTABLE cumulative-FAR snapshot; bounds only"),
              open(f"{OUT}/partial_meta.json","w"),indent=1)
