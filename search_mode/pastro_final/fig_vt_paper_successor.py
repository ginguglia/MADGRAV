...
Numerator = vt_relabel_comoving_successor.json:vt_comoving_srcframe_gpc3yr (four-epoch
...
Out: figures/vt_search/vt_vs_mass_paper.{pdf,png} + vt_paper_numbers_successor.json
"""
import json, os
import numpy as np
import matplotlib
...
rel = json.load(open(f"{HERE}/vt_relabel_comoving_successor.json"))
cmp_ = json.load(open(f"{HERE}/vt_compare_pipelines.json"))
...
for r in RUNS:
    ...
    neff = np.array(cmp_["neff_srcframe"][r]["support"], float)
...
axE.set_xlabel(r"$M_{\rm tot}$ (detector frame) [$M_\odot$]"); axE.set_ylabel("volume-averaged efficiency")
...
for ext in ("pdf", "png"):
    fig.savefig(f"{FIGDIR}/vt_vs_mass_paper_successor.{ext}", dpi=200, bbox_inches="tight")
...
num = dict(T_obs_yr=T, per_run_vt=({r: vt[r].tolist() for r in RUNS}), o3=o3.tolist(), total=tot.tolist(),
           ...
           masked_bins={r: [int(k) for k in np.where(np.array(cmp_["neff_srcframe"][r]["support"]) < NEFF_MIN)[0]] for r in RUNS})
json.dump(num, open(f"{HERE}/vt_paper_numbers_successor.json", "w"), indent=1)
print(json.dumps({k: num[k] for k in ("T_obs_yr", "peak_o3a", "peak_o3", "peak_total", "masked_bins")}, indent=1))
print(f"[fig_vt_paper] -> {FIGDIR}/vt_vs_mass_paper.pdf/.png")
