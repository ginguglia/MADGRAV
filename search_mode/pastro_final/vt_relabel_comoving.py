...
if "cnn" in SUF:                    # _x1cnn and _x1cnnveto both score the inj_cnn campaign
    # the CNN-scored campaign (per-injection cnn_hm/cnn_lm, O3a extended to 21 segments); the raw
    # templates MUST come from the same files that inj_scored{SUF} was built from -- G2 enforces it.
    # SM_INJ_CAMPAIGN names the injection campaign the scoring was built from; the default keeps
    # the old suffix-sniffing behaviour so earlier suffixes rebuild byte-identically.
    _ic = os.environ.get("SM_INJ_CAMPAIGN", "inj_fixed" if "fix" in SUF else "inj_cnn")
    INJ_DIRS = {r: [f"{SC}/{_ic}/{r.lower()}", f"{SC}/{_ic}/{r.lower()}_lowsnr"] for r in INJ_DIRS}


# SM_VT_FIXED_ASD=1 -> take the AS-RUN horizon from the leakage-free reference_psd_fixed_*.npz.
# vc_trunc = vmax(d_ref * rho_asrun / rho_th) depends ONLY on the as-run ASD (the release ASD
# cancels exactly between drel and c), so this file alone sets the reachable volume. With the
# frozen (0.25 s Welch) ASD the O4 horizon is understated by 3-4x at the VT peak and up to 56x at
# high mass. The search itself is NOT re-run: it whitens and then min-max-normalises, and is
# insensitive to the error (net_loc/net = 0.92-1.02 measured across all runs and masses).
FIXED_ASD = os.environ.get("SM_VT_FIXED_ASD", "0") == "1"
ASD_TAG = os.environ.get("SM_ASD_TAG", "fixed")


def load_asd(prep_dir, det, release):
    tag = "reference_psd_release" if release else (
        f"reference_psd_{ASD_TAG}" if FIXED_ASD else "reference_psd")
...
def main():
    ...
    for run in RUNS:
        ...
        z = np.load(f"{HERE}/inj_scored_{run.lower()}{SUF}.npz")
