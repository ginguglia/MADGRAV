#!/usr/bin/env python
"""RHO-MATCHED variant of vt_pipelines_target_zc.py (2026-09-02).

WHY: vt_relabel_comoving.py can now be run at a non-default horizon threshold
(SM_RHO_TH), but that restricts the MADGRAV NUMERATOR only - the comparator VT stayed
at rho_th=5, so every ratio simply fell by the numerator's own volume factor and the
comparison was not matched. This applies the SAME physical restriction to the pipeline
side: the release injection population is cut to sources whose H1+L1-only optimal
network SNR reaches rho_th.

H1+L1 ONLY, deliberately: MADGRAV is a coincident H1+L1 search, and its rho_net
(vt_relabel_comoving.rho_net) sums H1 and L1 alone. The release sets record per-detector
optimal SNRs, so the comparator cut is built as hypot(snr_H, snr_L) and never from the
released network value, which includes Virgo. Cutting on the HLV value would restrict
the comparator at a stricter physical threshold than the numerator and bias every O4
ratio low - the direction the exercise is trying to measure.

At SM_RHO_TH=5 (the default) the cut is inert only if the release sets contain no
injections below rho_hl=5; they do, so this script must NOT be used to regenerate the
accepted rho_th=5 products. It writes its own suffixed output and never overwrites them.
...
# SM_RHO_TH: H1+L1-only optimal network SNR floor applied to the release injections,
# matching vt_relabel_comoving.py's horizon threshold. SM_VT_OUT_SUF: output-only suffix
# so a variant never lands on top of the accepted vt_pipelines_target_zc.json.
RHO_TH = float(os.environ.get("SM_RHO_TH", "5.0"))
OSUF = os.environ.get("SM_VT_OUT_SUF", "")
...
from vt_pipelines_gwtc import D as D_G3
...
def _o3_rho_hl(run, tag):
    """H1+L1 optimal network SNR per injection for one GWTC-3 release population."""
    gps = {"O3a": "1238166018-15843600", "O3b": "1256655642-12905976"}[run]
    with h5py.File(f"{D_G3}/endo3_{tag}-LIGO-T2100113-v12-{gps}.hdf5", "r") as f:
        i = f["injections"]
        return np.hypot(i["optimal_snr_h"][:], i["optimal_snr_l"][:])
...
def main():
    ...
    for run in ("O3a", "O3b"):
        ...
        # H1+L1-only optimal network SNR of each release injection, read straight from
        # the GWTC-3 sets (load_o3 does not carry these columns; the accepted loader is
        # left untouched). optimal_snr_v is ignored by construction - see the header.
        rho_hl = np.concatenate([_o3_rho_hl(run, tag) for tag in ("bbhpop", "imbhpop")])
        assert len(rho_hl) == len(w_t), f"{run}: rho_hl length {len(rho_hl)} != {len(w_t)}"
        rho_ok = rho_hl >= RHO_TH
        print(f"[{run}] rho_hl >= {RHO_TH}: {rho_ok.mean():.3%} of injections kept")
        iv, T_ours = our_segments(run)
        ins = in_segments(gps, iv)
        run_out = {"T_ours_yr": T_ours, "f_um": F_UM_DESIGN, "pipelines": {}}
        for p in pipelines:
            det = (ifars[p] > 1.0 / FAR_THR) & rho_ok
    ...
    ev = f5["events"]
    c5 = {k: ev[k][:] for k in
          ("mass1_source", "mass2_source", "z", "luminosity_distance", "snr_H", "snr_L",
    ...
    # GWTC-5 ships snr_H/snr_L (optimal) alongside observed_snr_*; the optimal pair is
    # the analogue of the numerator's bank-template rho_net, and Virgo is excluded.
    rho_ok5 = np.hypot(c5["snr_H"], c5["snr_L"]) >= RHO_TH
    print(f"[O4] rho_hl >= {RHO_TH}: {rho_ok5.mean():.3%} of injections kept")
    ...
    res["rho_th_hl"] = RHO_TH
    json.dump(res, open(f"{HERE}/vt_pipelines_target_zc{OSUF}.json", "w"), indent=1)
    print(f"[done] -> {HERE}/vt_pipelines_target_zc{OSUF}.json  (RHO_TH={RHO_TH})")
