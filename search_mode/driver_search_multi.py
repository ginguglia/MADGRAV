...
def require_complete(avail,segments,what="blindscan"):
    """Refuse to score a partial segment set. In O4b, 101 stream shards died on one bad GPU node and the
    FAR stage (wired afterany) silently ran on 785/1086 segments -- a quarter of the run vanished with no
    error, and was only caught months later by auditing a figure's denominator. Set SM_ALLOW_PARTIAL=1 to
    run a deliberate subset; it is then recorded in the output so the result is never mistaken for full."""
    miss=len(segments)-len(avail)
    if not miss: return
    msg=(f"[{what}] INCOMPLETE: {len(avail)}/{len(segments)} segments have streams ({miss} missing). "
         f"Streams dir={S_BG}. Rebuild the missing streams, or set SM_ALLOW_PARTIAL=1 to proceed knowingly.")
    if not os.environ.get("SM_ALLOW_PARTIAL"): raise SystemExit(msg)
    print(f"[{what}] WARNING (SM_ALLOW_PARTIAL=1) {msg}",flush=True)
def load_seg(a,nm):
...
def main():
    ...
    require_complete(avail,SEG["segments"],"search")
    ...
    evloc={ev:(i,int(np.abs(segs[i]["gps"]-g0).argmin())) for ev,g0 in EVENTS.items() for i,s in enumerate(segs) if s["gps"][0]<=g0<=s["gps"][-1]}   # GPS-containment: segments are GPS-named o3a_*, not event-named, so name-equality never matched -> proper cross-fit
