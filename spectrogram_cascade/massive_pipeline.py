...
class MassiveEventPipeline:
    def __init__(self, calib_path=DEF_CALIB, prep=DEF_PREP, device=None):
        ...
        # latent_channels: absent/null in the frozen calibration -> production overcomplete latent (bit-identical)
        self.model = ip.BaselineCAE(dropout=0.20, latent_channels=c.get("latent_channels")).to(self.device)
