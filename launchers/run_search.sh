...
# floor 4.0 / net-sigma 4.0 / per-arm / blind-floor / COH_CEIL 0.999.
...
export SM_COH_CEIL="${SM_COH_CEIL:-0.999}"
...
SM_NSHARD=$N BLIND_DEV="$DEV" bash "$HERE/run_merge.sh"; mrc=$?
echo "[madgrav] merge finished rc=$mrc -> $OUT (blindscan.json / detections.json / survivors_bg.json)"
exit $mrc   # propagate merge failure so SLURM/afterok/supervisor never sees a failed merge as success
