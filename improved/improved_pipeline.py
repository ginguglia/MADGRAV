...
class Config:
    ...
    margin: float = 3.0
    latent_channels: int = 0        # 0 -> production overcomplete latent; k>0 -> compressive bottleneck k*32*16
...
class BaselineCAE(nn.Module):
    # latent_channels=None -> the production overcomplete latent (128*32*16 = 65,536, 2x the input).
    # latent_channels=k    -> a COMPRESSIVE bottleneck of k*32*16 dims, inserted as 1x1 convs between
    # pool3 and the flatten (and mirrored before unpool1, which needs 128 channels back to match its
    # switch indices). k=32 -> 16,384; k=20 -> 10,240; k=8 -> 4,096; k=1 -> 512, matching the
    # 2026-06-11 ablation's C_k naming. Default None keeps the deployed model bit-identical.
    def __init__(self, dropout=0.20, latent_channels=None):
        super().__init__()
        d = dropout
        self.latent_channels = latent_channels
        lc = 128 if latent_channels is None else int(latent_channels)
        self.latent_dim = lc * 32 * 16
        ...
        self.unflatten = nn.Unflatten(1, (lc, 32, 16))
        self.classifier = nn.Linear(self.latent_dim, 1)
        if latent_channels is not None:
            self.bott_down = nn.Conv2d(128, lc, 1)
            self.bott_up = nn.Conv2d(lc, 128, 1)
...
def main():
    ...
    parser.add_argument("--latent-channels", type=int, default=0,
                        help="0 = production overcomplete latent (65,536); k>0 = compressive bottleneck of k*32*16")
    ...
    model = BaselineCAE(dropout=cfg.dropout, latent_channels=(cfg.latent_channels or None)).to(device)
    unsup_history = None
    joint_history = None
    ...
    if args.eval_only:
        if unsup_only_mode:
            ...
            if os.path.exists(unsup_ckpt):
                unsup_eval_model = BaselineCAE(dropout=cfg.dropout, latent_channels=(cfg.latent_channels or None)).to(device)
